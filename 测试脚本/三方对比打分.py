...
被测答案：{df.at[index, answer_col]}
        ...
        df.at[index, f"{score_col_prefix}_事实一致性"] = response["事实一致性"]
        df.at[index, f"{score_col_prefix}_无矛盾性"] = response["无矛盾性"]
        df.at[index, f"{score_col_prefix}_意图覆盖度"] = response["意图覆盖度"]
...
if __name__ == "__main__":
    judge_single("../测试用例/0311模型生成测试用例_jiuwen_select.xlsx",
                 "../测试用例/0311模型生成测试用例_jiuwen_select.xlsx",
                 answer_col="answer_jiuwen_20轮",
                 score_col_prefix="score_jiuwen")
